...
function BusinessReListJob({ route,navigation }) {
  ...
  const [date, setDate] = useState(new Date(route.params.job.start_date));
  ...
  const [description, setDescription] = useState(route.params.job.position_desc);
    ...
    postFormData("/update_job", form)
      ...
      .then((json) => {
        ...
        if(json.status_code==200){
          Alert.alert("","Job has been re-listed successfully");
          navigation.goBack()
          ...
          <View style={{ flex: 1, width: "100%", paddingTop: 20 }}>
            ...
            >
              ...
              >
                ...
                >
                  {date==null?<Text style={{ width: 120,                    color: "#666",
 }}>{strings.START_DATE}</Text>:
                  <Text style={{ width: 120 }}>{date==null?strings.START_DATE:formatDate(date)}</Text>}
                </TouchableOpacity>
              </View>
            </View>

            <View style={{ flex: 3, alignItems: "center", marginHorizontal: "5%" ,marginTop:20}}>
            <TouchableOpacity 
          style={{width:'100%', backgroundColor: '#4834A6' ,paddingTop: 12, paddingBottom: 12, borderRadius: 50,}}
