...
function BusinessSubmittedApplication({ route, navigation }) {
  ...
  const [headerName] = useState(route.name=="BusinessEmployeesApplication"?strings.EMPLOYEES:strings.SUBMITTED_APPLICATIONS)
    ...
    return (
      <TouchableOpacity key={item.id} onPress={() => {
          ...
          tempUser.job_detail = [item.job_detail];
                          navigation.navigate('BusinessSeekerProfile',{seeker:tempUser,job:item})

          }else{
            tempUser = item;
            tempUser.job_detail = [route.params.job];
                navigation.navigate('BusinessSeekerProfile',{business:user,seeker:tempUser,job:item})
        ...
        <View  style={{
          ...
          <View style={{width: '20%',borderRadius:20}}>
            <Image 
              source={{uri: (item.user?item.user.avatar_image:item.avatar_image)}} 
          ...
          <View style={{width: '80%'}}>
            <View style={{flex: 1}}>
              <Text style={{fontSize: 20,color:'#000'}}>{item.user?item.user.first_name:item.first_name} {item.user?item.user.last_name:item.last_name}</Text>
              <Text>{item.position_name?item.position_name:route.params.job.position}</Text>
              <View style={{flexDirection:'row',alignItems:'center'}}>
                  <Image source={require('../assets/ic_location.png')} style={{height:10,width:10,marginRight:5}} />
  ...
  return (
    ...
    >
          ...
          >
          <View style={{ postion:'absolute', left:10}}>
